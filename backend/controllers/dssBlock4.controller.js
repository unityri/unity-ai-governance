...
function resolveFixturePath(prototypeDir, fixture = "partial") {
  ...
  return path.join(prototypeDir, "fixtures", `block34_${fixtureName}_scenario`, "evidence_package.json");
}

async function runBlock4Assessment(payload = {}) {
  const prototypeDir = resolvePrototypeDir();
  const runner = path.join(prototypeDir, "block4_respond.py");

  if (!fs.existsSync(runner)) {
    const error = new Error("DSS Block 4 runner was not found.");
  ...
  if (evidencePackage) {
    ...
    if (!fs.existsSync(inputPath)) {
      const error = new Error("DSS Block 4 fixture was not found.");
      error.prototype_dir = prototypeDir;
...
from block4_respond import build_results_payload
...
`;
  ...
  return {
    success: true,
    runner: "prototype/block4_respond.py",
