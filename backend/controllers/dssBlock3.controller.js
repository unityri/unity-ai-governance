...
async function runBlock3Assessment(payload = {}) {
  ...
  const runner = path.join(prototypeDir, "block3_monitor.py");

  if (!fs.existsSync(runner)) {
    const error = new Error("DSS Block 3 runner was not found.");
  ...
  if (evidencePackage) {
    ...
    if (!fs.existsSync(inputPath)) {
      const error = new Error("DSS Block 3 fixture was not found.");
      error.prototype_dir = prototypeDir;
...
from block3_monitor import build_results_payload
...
`;
  ...
  return {
    success: true,
    runner: "prototype/block3_monitor.py",
